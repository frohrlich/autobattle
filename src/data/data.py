...
def get_random_item_identifier():
    return random.choice(list(item_infos.keys()))
...
def get_sell_price_by_quality(quality):
...
def get_buy_price_by_quality(quality):
    for key, value in buy_prices.items():
        if key == quality.name:
            return value
    return None
